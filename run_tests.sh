...
case $test in 
   ...
   3) #Hashtable Postgres SECD (hashtable is lost if set up in separate command)
      psql -p $PSQL_PORT -o out.txt -q -c "\timing on" -c "\i Hashtables/SECD/secd.sql" -c "SELECT id,val,n FROM root_terms AS _(id,t), LATERAL evaluate(t) AS r(val,n);"
      ;;
   4) #Hashtable Postgres Krivine
      psql -p $PSQL_PORT -o out.txt -q -c "\timing on" -c "\i Hashtables/Krivine/krivine.sql" -c "SELECT id,val,n FROM root_terms AS _(id,t), LATERAL evaluate(t) AS r(val,n);"
      ;;
